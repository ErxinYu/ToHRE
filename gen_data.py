...
out_path = "./data/57K-combined-joined-first/"
#out_path = "./data"
case_sensitive = False
if not os.path.exists(out_path):
    os.mkdir(out_path)
train_file_name = in_path + 'train_combined_joined_first.json'
test_file_name = in_path + 'test_combined_joined_first.json'
...
def init(file_name, word_vec_file_name, rel2id_file_name, max_length = 120, case_sensitive = False, is_training = True):
    ...
    for i in range(len(ori_data)):
        if  i%10000 == 0:
            print(i)
        sen = ori_data[i]
        h_entity = sen["head"]["word"].lower().split()[0]
        t_entity = sen["tail"]["word"].lower().split()[0]
        if h_entity in word2id:
            h_entity_word[i][0] = word2id[h_entity]
        if t_entity in word2id:
            t_entity_word[i][0] = word2id[t_entity]
        # sen_label 
        #rule 1
    ...
    np.save(os.path.join(out_path, name_prefix + '_h_entity_word.npy'), h_entity_word)
    np.save(os.path.join(out_path, name_prefix + '_t_entity_word.npy'), t_entity_word)
